"""Render the figures and summary table the thesis reports from a results.json.

Lives in the package rather than in scripts/ so that `prdbench report` and
`scripts/make_figures.py` are the same code path: a figure in the write-up is
traceable to one results file and one commit, whichever way it was produced.
"""
